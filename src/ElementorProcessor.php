...
class ElementorProcessor {
    ...
    {
        $this->items = $result;
        $data = $this->injectElementorData('meta/_elementor_data/', $this->data);
        $this->items['meta/_elementor_data'] = trim(json_encode(json_encode($data, JSON_THROW_ON_ERROR | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP), JSON_THROW_ON_ERROR), '"');
    ...
    protected function injectElementorData($prefix, &$data): array
    {
        foreach ($data as $componentIndex => &$components) {
            ...
            if (isset($components['elements'])) {
                $components['elements'] = $this->injectElementorData($myPrefix . '/', $components['elements']);
            }
            if (isset($components['settings'])) {
                foreach ($components['settings'] as $settingIndex => $setting) {
                    ...
                    if (is_array($setting)) {
                            ...
                            foreach ($setting as $optionIndex => $option) {
                                if (is_array($option)) {
                                    ...
                                    foreach ($options as $optionsArrayIndex => $optionValue) {
                                        ...
                                        } else if (isset($this->items[$key]) && in_array($optionsArrayIndex, ElementorFilter::$allowKeys, true)) {
                                            $data[$componentIndex]['settings'][$settingIndex][$optionIndex][$optionsArrayIndex] = $this->items[$key];
                                            unset($this->items[$key]);
                                        }
                                    }
                                } else {
                                    $key = $myPrefix . '/' . $settingIndex . '/' . $optionIndex;
                                    if (isset($this->items[$key]) && in_array($optionIndex, ElementorFilter::$allowKeys, true)) {
                                        $data[$componentIndex]['settings'][$settingIndex][$optionIndex] = $this->items[$key];
                                        unset($this->items[$key]);
                                    }
                                }
                            }
                        ...
                        if (isset($this->items[$key]) && in_array($settingIndex, ElementorFilter::$allowKeys, true)) {
                            $data[$componentIndex]['settings'][$settingIndex] = $this->items[$key];
                            unset($this->items[$key]);
                        }
                    }
                }
